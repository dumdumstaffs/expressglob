import { ensureArray } from "@shared/helpers";
import { ChangeKey } from "@shared/types/transformers";
import { Query } from "mongoose";
import { FilterQueryParams } from "./query";
  ...
  private filterExpressionFromQuery() {
    ...
    return (
      Object.keys(this.query)
        ...
        .filter((queryKey) => !!this.query[queryKey as keyof typeof this.query])
        // remove query param not in the filter fields
        .filter((queryKey): queryKey is keyof T & string =>
          this.fields.some(({ field }) => field === queryKey),
        )
        // return filter fields with correct type
        .reduce(
          (acc, curr) => {
            const queryValue = ensureArray(
              this.query[curr as keyof typeof this.query],
            )[0];
