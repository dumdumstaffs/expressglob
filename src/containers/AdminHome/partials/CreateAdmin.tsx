...
export const CreateAdmin = () => {
  ...
  const {
    ...
    formState: { errors },
    setFocus,
  } = useForm<z.input<typeof AdminSchema.create>>({
    ...
    <div className="border-solid border-0 border-b border-gray-200 pb-12 px-1 sm:px-0">
      ...
      <form onSubmit={onSubmit} className="tw-form">
        <h2 className="text-center text-xl font-thin sm:mx-[75px] mt-[20px] mb-[25px]">
          Enter up to 30 of your Broadway Transit tracking, door tag, or
          Broadway Transit Office order numbers (one per line).
        </h2>
        <Input
          label="Email"
          {...register("email")}
        ...
        <p className="text-red-400 !my-6 text-center">
          {createAdminMutation.isError && createAdminMutation.error.message}
        </p>

        <div className="text-right">
          <Button type="submit" loading={createAdminMutation.isLoading}>
            Create New
          </Button>
        </div>
      </form>
