...
export const CreateParcel = () => {
  ...
  const createShipmentMutation = trpc.shipment.create.useMutation();
  ...
  const {
    ...
    setFocus,
    watch,
  } = useForm<z.input<typeof ShipmentSchema.create>>({
    resolver: zodResolver(ShipmentSchema.create),
  });
    ...
    createShipmentMutation.mutate(data, {
      onSuccess(data) {
        router.push(
          "/manager/track" +
            "?" +
            new URLSearchParams({ trackingId: data.trackingId }),
        );
      },
    });
  });
