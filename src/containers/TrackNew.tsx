...
export const TrackNew = ({ url, onTrack }: { url: string, onTrack: () => void }) => {
    ...
    const { handleSubmit, register, formState: { errors, isSubmitting }, setFocus } = useForm<TrackDto>({
        resolver: yupResolver(TrackSchema.track)
    })
        ...
        <div className="border-solid border-0 border-b border-gray-200 pb-12 px-1 sm:px-0">
            ...
            <form onSubmit={onSubmit} className="mx-auto w-full max-w-[600px]">
                ...
                <div className="text-right">
                    <Button type="submit" loading={isSubmitting}>Track</Button>
                </div>
            </form>
        </div>
    )
