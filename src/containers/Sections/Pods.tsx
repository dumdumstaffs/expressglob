...
const ITEMS = [
    {
        ...
        desc: "Help keep your deliveries safe when you ask us to hold packages at one of thousands of nearby OptionDelivery® locations.Track your package to begin."
    },
    {
        ...
        <div className="pods_container">
            <div className="fxg-pods fxg-pods--fixed  fxg-tablet--hide fxg-desktop--hide">
                <div className="fxg-wrapper container">
                    ...
                    <div className="row fxg-swipe fxg-swipe--active" style={{ height: 440, visibility: "visible" }}>
                        <div className="fxg-swipe__wrapper" style={{ width: width * 3 + "px", left: "0px" }}>
                            {
                                ITEMS.map((item, i) => (
                                    <CarouselItem
                                        key={i}
                        ...
                        <ul className="fxg-slider__indecators">
                            {
                                ITEMS.map((_, i) => (
                                    <li key={i} className={`fxg-slider__indecator ${active === i ? "active" : ""}`} onClick={() => setActive(i)} />
                                ))
                            }
                        </ul>
                    </div>
...
const CarouselItem = ({ title, image, href, linkText, desc, width, index, active, next }: CarouselItemProps) => (
    <div
        ...
        <div style={{ marginBottom: "80px" }}>
            ...
            <div className="column_control_v1">
                <div className="row" style={{ backgroundColor: "" }}>
                    {ITEMS.map((item, i) => <RowItem key={i} {...item} />)}
                </div>
            </div>
            <div className="column_control_v1">
                <div className="row" style={{ backgroundColor: "" }}>
                    {ITEMS.map((item, i) => <RowButton key={i}  {...item} />)}
                </div>
            </div>
        </div>
    )
...
const RowItem = ({ title, image, desc }: Pick<typeof ITEMS[number], "title" | "image" | "desc">) => (
...
const RowButton = ({ href, linkText }: Pick<typeof ITEMS[number], "href" | "linkText">) => (
    <div
        className="fxg-col col-sm-4 fxg-mobile--hide">
        <div style={{ backgroundColor: "" }}>
