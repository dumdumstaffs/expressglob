import { Image } from "@shared/types/shipment";
    ...
    <div className="pods_container">
      <div className="fxg-pods fxg-pods--fixed">
        <div className="fxg-wrapper container">
          ...
          >
            <div
              className="fxg-swipe__wrapper mb-12"
              style={{ width: width * images.length, left: "0px" }}
            >
              {images?.map((image, i) => (
                <CarouselItem
                  key={image.id}
            ...
            <ul className="fxg-slider__indecators">
              {images?.map((_, i) => (
                <li
                  key={i}
                  className={`fxg-slider__indecator w-4 h-4 !rounded-full ${
                    active === i ? "active" : ""
                  }`}
                  onClick={() => setActive(i)}
                />
