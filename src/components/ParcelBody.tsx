...
const ParcelBody = ({ shipment }: { shipment: Shipment }) => {
    ...
    return (
        <div className="text-center mt-12 mx-auto w-full sm:w-8/12">
            ...
            <div onClick={() => setStatusUpdates(false)} className={`bg-fedex-bg text-gray-800 text-sm pl-14 pr-4 py-4 text-left relative cursor-pointer ${!statusUpdates ? "hidden" : ""}`}>
                <p className="absolute top-4 left-6 border-solid border rounded-full w-5 h-5 text-sm font-bold text-center">i</p>
                Live notifications is already setup for this package, you will be notified when this package arrives its destination.
